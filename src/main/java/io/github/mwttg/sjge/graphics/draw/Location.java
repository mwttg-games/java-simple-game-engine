package io.github.mwttg.sjge.graphics.draw;

public interface Location {

  String LIGHT_COLOR = "pointLight.color";
  String LIGHT_GAMMA = "pointLight.gamma";
  String LIGHT_POSITION = "pointLight.position";
  String MATERIAL_AMBIENT = "material.ambient";
  String MATERIAL_DIFFUSE = "material.diffuse";
  String MATERIAL_SPECULAR = "material.specular";
  String MATERIAL_SPECULAR_EXPONENT = "material.specularExponent";
  String MODEL_MATRIX = "modelMatrix";
  String MODEL_VIEW_PERSPECTIVE_MATRIX = "modelViewPerspectiveMatrix";
  String NORMAL_MATRIX = "normalMatrix";
  String PROJECTION_MATRIX = "projectionMatrix";
  String TEXTURE_SAMPLER = "textureSampler";
  String VIEW_MATRIX = "viewMatrix";
}
